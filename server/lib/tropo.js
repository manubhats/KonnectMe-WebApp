...
export let call = (initiator_name, recipient_name, recipient_phone_number, message) => {
	let request = https.get(`${url}${voice_token}&initiator_name=${initiator_name}&recipient_name=${recipient_name}&recipient_phone_number=${recipient_phone_number}&message=${message}`);
	request.end();
}
